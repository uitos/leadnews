...
public class WmMaterialServiceImpl extends ServiceImpl<WmMaterialMapper, WmMaterial>
    ...
    public ResponseResult uploadPicture(MultipartFile multipartFile) {
        ...
        try {
            ...
            String imageUri = fileStorageService.uploadHtmlFile("", filename, multipartFile.getInputStream());
            //2.保存素材到DB
            WmMaterial wmMaterial = new WmMaterial();
            wmMaterial.setUserId(wmUserId.intValue());
            wmMaterial.setUrl(imageUri);
            wmMaterial.setType((short) 0);
            wmMaterial.setIsCollection((short) 0);
            wmMaterial.setCreatedTime(new Date());
            boolean flag = save(wmMaterial);
            if (!flag) {
                //保存素材到数据库失败，为了保存数据一致性，上传MinIO成功的图片，也应该删除
                fileStorageService.delete(imageUri);
